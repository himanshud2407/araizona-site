...
const AboutSection: React.FC = () => {
  useEffect(() => {
    ...
    if (bgEl) bgEl.style.backgroundImage = `url(${(aboutBg as any).src || aboutBg})`;
  }, []);
      ...
      <div className="container">
        <div className="sec-title about-sec-title mb-75">
          ...
          <h2 className="title">
            We helping
            <span>
              <img src={(gif1 as any).src || gif1} alt="animation" />
            </span>{" "}
            grow businesses and
            <span>
              <img src={(gif2 as any).src || gif2} alt="animation" />
            </span>
            scale with smart, data-driven AI agency{" "}
            <span>
              <img src={(gif3 as any).src || gif3} alt="animation" />
            </span>
          </h2>
        </div>
      </div>
            ...
            <div key={idx} className="xb-about-img-item img-hove-effect">
              <div className="xb-img">
                <Link href="/service-details">
                  <img src={(img as any).src || img} alt={`about-${idx + 1}`} />
                </Link>
              </div>
            </div>
          ))}
