...
const IndustriesSection: React.FC = () => {
  useEffect(() => {
    ...
    if (section) {
      section.style.backgroundImage = `url(${(bgImg as any).src || bgImg})`;
    }
  }, []);
    ...
    <section className="industries pb-165 bg_img bg-wrap">
      <div className="container">
        <div className="sec-title sec-title-center text-center mb-50">
          <span className="sub-title mb-15">Industry Solutions</span>
          <h2 className="title">Built for Industries That Can't Afford Downtime</h2>
        </div>
      </div>
            ...
            {industries.map((item, index) => (
              <div className="xb-industries-item xb-border" key={index}>
                <div className="xb-icon">
                  <img src={(item.icon as any).src || item.icon} alt={item.title} />
                </div>
